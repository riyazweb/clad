...
def main():
    ...
    while True:
        user_input = "write a motivational quote in 10 words for :" + input("You: ") 
        ...
        # Load the audio file
        audio_filename = "audio.mp3"  # Replace with the actual audio file path
        audio = AudioFileClip(audio_filename)

        # Load the image
         # Load the image
        img_clip = ImageClip(img_filename, duration=audio.duration)  # Match image duration with audio

        # Calculate the minimum duration between the image and audio clips
        min_duration = min(img_clip.duration, audio.duration)

        # Set the duration of the final video to the minimum duration
        final_video_clip = img_clip.subclip(0, min_duration).set_audio(audio.subclip(0, min_duration))

    

    # Specify the frames per second (fps) for the video
        final_video_clip.fps = 30  # Adjust this value if needed
        # Resize the video to 480x720 pixels
        final_video_clip = final_video_clip.resize(width=480, height=720)

        # Position the image at the center of the video
        final_video_clip = final_video_clip.set_position(("center", "center"))

        # Export the final video
        video_filename = f"{dog}.mp4"
